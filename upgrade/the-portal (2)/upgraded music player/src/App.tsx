...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 1. Inject the Web Playback SDK script dynamically
    ...
    // 2. Attach global callback that runs once the SDK is loaded
    (window as any).onSpotifyWebPlaybackSDKReady = () => {
      if (spotifyPlayerRef.current) return; // avoid duplicate initialization
      ...
      player.addListener("player_state_changed", (state: any) => {
        ...
        // Pause automatically if in preview mode and threshold of 30 seconds is reached
      ...
      player.addListener("ready", ({ device_id }: { device_id: string }) => {
        ...
        // Auto-transfer active player session to our Web Playback browser instance
    ...
    // If Spotify script was already loaded and window.Spotify object exists
    if ((window as any).Spotify && !spotifyPlayerRef.current) {
      (window as any).onSpotifyWebPlaybackSDKReady();
    }

    return () => {
      // Disconnect player on unmount or token invalidate
    };
  }, [spotifyToken]);
      ...
      timerId = setInterval(() => {
        setCurrentTime((prev) => {
          ...
          if (previewMode && nextTime >= 30) {
            setIsPlaying(false);
            // Stop the real SDK player if active
            if (spotifyPlayerRef.current) {
  ...
  const handleCreatePlaylist = (name: string, description?: string, tracks?: Track[]) => {
    ...
    // Focus the new playlist immediately
    setActivePlaylist(newPlaylist);
    setActivePlaylistId(newPlaylist.id);
    setActiveTab("explore"); // Transition to explore tab to instantly see compilation
  };
      ...
      try {
        if (type === "track") {
          ...
          if (playlistRes.ok) {
            ...
            while (nextUrl && pagesFetched < 15) { // Fetch up to 750 tracks
            ...
            if (tracks.length > 0) {
              ...
              // Add to custom playlists if not already in there
  ...
  const handlePlayTrack = (track: Track) => {
    ...
    setIsLiked(false);
    setShowLyricsPanel(true); // Automatically show player panel so user can access iframe play buttons

    // If connected to Spotify, trigger active device playback remotely
    if (spotifyToken) {
  ...
  // Synchronize volume change to active Spotify device (with debouncing to prevent rate limits)
  useEffect(() => {
  ...
  const handleAddToQueue = (track: Track) => {
    ...
    // Optionally switch to the queue tab/panel to give instant feedback
    setRightPanelTab("queue");
  };

  return (
    <div id="app-window" className="w-full h-screen bg-gradient-to-br from-[#0c041a] via-[#04020a] to-[#010103] flex flex-col overflow-hidden text-[#e0dcd0] font-sans antialiased">
        ...
        {showLyricsPanel && (
          <aside className="w-80 flex flex-col border-l border-[#8b5cf6]/15 bg-[#06030d]/90 backdrop-blur-md p-4 space-y-4 shrink-0 overflow-y-auto scrollbar-none select-none fixed lg:relative right-0 top-0 bottom-24 lg:bottom-0 h-[calc(100vh-6rem)] lg:h-auto z-40 shadow-2xl lg:shadow-none animate-fade-in">
            ...
            {rightPanelTab === "lyrics" ? (
              ...
              /* Play Queue */
              <div className="flex-1 flex flex-col min-h-[250px] overflow-hidden bg-[#050505]/40 border border-white/5 rounded-2xl p-3 space-y-3 font-sans">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                  {queue.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center p-4">
                      ...
                      <p className="text-[10px] text-zinc-600 mt-1">Click the 3 dots on any song and select &ldquo;Add to Queue&rdquo;.</p>
                    </div>
                  ) : (
                    queue.map((track, i) => (
                      <div
                      ...
                      >
                        <div 
                          ...
                          onClick={() => {
                            // Play immediately: remove from queue, make current
      ...
      <footer className="h-24 bg-gradient-to-r from-[#0c041a] to-[#04020a] border-t border-[#8b5cf6]/20 px-8 flex items-center justify-between shrink-0 select-none">
        ...
        <div className="flex flex-col items-center gap-2.5 flex-1 max-w-xl">
          <div className="flex items-center gap-8">
            ...
            <button
              ...
              className="w-12 h-12 rounded-full border border-[#8b5cf6] flex items-center justify-center text-[#8b5cf6] hover:bg-[#8b5cf6] hover:text-white transition shadow-[0_0_15px_rgba(139,92,246,0.25)] shrink-0"
            >
        ...
        <div className="flex items-center justify-end gap-6 w-1/4">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-bold font-mono tracking-wider uppercase transition-all duration-300 border ${
              previewMode
