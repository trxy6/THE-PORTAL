...
const inlineStyles = `
  ...
  /* Galaxy Cosmic Theme Skins */
  .galaxy-shell {
    background: radial-gradient(circle at 50% 30%, rgba(139, 92, 246, 0.35) 0%, rgba(67, 56, 202, 0.15) 50%, rgba(10, 6, 22, 0.98) 100%);
    box-shadow: 
      0 20px 50px rgba(0, 0, 0, 0.9),
      0 0 35px rgba(139, 92, 246, 0.25),
      inset 0 0 25px rgba(79, 70, 229, 0.1);
    border: 2px solid rgba(139, 92, 246, 0.3);
  }
  .galaxy-bezel {
    background: radial-gradient(circle at center, #170d30 0%, #080314 100%);
    box-shadow: 
  ...
  .galaxy-screen {
    background: #020008;
    color: #3fd9c7;
    text-shadow: 0 0 6px rgba(63, 217, 199, 0.85);
  }
  .galaxy-btn-black {
  ...
  .galaxy-btn-blue {
    background: rgba(30, 41, 88, 0.75);
    border: 1px solid rgba(59, 130, 246, 0.35);
    color: #93c5fd;
  }
  .galaxy-btn-blue:hover {
  ...
  .galaxy-btn-operator:hover {
    border-color: rgba(168, 85, 247, 0.85);
    box-shadow: 0 0 10px rgba(168, 85, 247, 0.65);
  }
  .galaxy-text-cyan {
    color: #3fd9c7;
    text-shadow: 0 0 4px rgba(63, 217, 199, 0.6);
  }
  .galaxy-text-purple {
    color: #c084fc;
...
export default function App() {
  ...
  const formatImpliedMultiplication = (expr: string) => {
    return expr
      .replace(/(\d+)([a-zA-Z╧Ç╬╕n])/g, '$1 * $2')
      .replace(/([a-zA-Z╧Ç╬╕n])(\d+)/g, '$1 * $2')
      .replace(/\)([\w(╧Ç])/g, ') * $1')
      .replace(/([a-zA-Z╧Ç╬╕n])\(/g, '$1 * (');
  };
  ...
  // Feedback ideas persistence and sync
  ...
  const handleOracleConsult = () => {
    ...
    setTimeout(() => {
      const oracleAnswers = [
        "The runes glow brightly: The path ahead is clear. Go for it! Γ£¿",
        "Shadows cloud the future. Patience is key. Wait for a sign. ΓÅ│",
        "By all indications, the celestial alignments favor this option! ≡ƒæì",
        "Warning: The energies are highly chaotic. Steer clear for now. ≡ƒ¢æ",
      ...
      setOracleAnswer(`"${q}" ΓåÆ ${oracleAnswers[index]}`);
      haptic([100, 50, 100]);
    }, 1200);
  };
  ...
  const addToParlaySlip = (event: any, competition: any, competitor: any) => {
    ...
    if (isGameIn) {
      toast("ΓÜá∩╕Å You already have a pick from this game in your parlay!");
      return;
    }
  ...
  const saveParlay = () => {
    if (parlaySlip.length === 0) {
      toast("ΓÜá∩╕Å Your parlay slip is empty! Pick some teams first.");
      return;
    }
    ...
    toast("≡ƒÅå Parlay successfully saved to tracker!");
  ...
  const refreshParlays = async () => {
    toast("≡ƒöä Refreshing parlay outcomes across networks...");
    ...
    toast("Γ£ô Parlays updated with latest public scores.");
  };
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (!uId || !pinVal) {
      setAuthError("ΓÜá∩╕Å Please fill in all credentials!");
      return;
    }
    ...
    if (matched && matched.pin === pinVal) {
      ...
      setAuthError("ΓÜá∩╕Å Credentials invalid or PIN mismatch!");
    }
  };
  ...
  const handleSignUpSubmit = (e: React.FormEvent) => {
    ...
    if (uId.length < 3) {
      setAuthError("ΓÜá∩╕Å User ID must be at least 3 characters!");
      return;
    }
    if (pinVal.length !== 4) {
      setAuthError("ΓÜá∩╕Å PIN must be exactly 4 digits!");
      return;
    }
    const rawUsers = localStorage.getItem('portal_users');
    const users = rawUsers ? JSON.parse(rawUsers) : [];
    ...
    if (alreadyExists) {
      setAuthError("ΓÜá∩╕Å User ID already claimed by another Traveler!");
      return;
    }
  ...
  useEffect(() => {
    ...
    function renderNotes() {
      ...
      if (notesArr.length === 0) {
        list.innerHTML =
          '<div class="empty-state"><span class="text-3xl filter saturate-50 select-none">≡ƒ¬╢</span>The page is blank ΓÇö write your first note</div>';
        return;
      }
    ...
    function renderTasks() {
      ...
      if (tasksArr.length === 0) {
        list.innerHTML =
          '<div class="empty-state"><span class="text-3xl filter saturate-50 select-none">≡ƒô£</span>Nothing on the list yet</div>';
        return;
      }
      tasksArr.forEach((task: any, idx: number) => {
        ...
        item.innerHTML = `
          <div class="flex items-center gap-2 min-w-0">
            <div class="task-check ${task.done ? 'done' : ''}" data-toggle="${idx}">${task.done ? 'Γ£ô' : ''}</div>
            ${priorityPill}
            <div class="task-text truncate ${task.done ? 'done' : ''}">${task.text}</div>
          </div>
          <div class="task-del shrink-0" data-del="${idx}">Γ£ò</div>
        `;
        list.appendChild(item);
      });
        ...
        c.addEventListener('click', () => {
          ...
          if (tasksArr[i].done && !oldDone) {
            ...
            if (prio === 'routine') {
              ...
              updateHabitDisplays();
              toast(`Routine complete! Streak increased: ${curStreak} days ≡ƒöÑ`, 'success');
            }
    ...
    function updatePomoDisplay() {
      ...
      if (pomoStateLabel) {
        pomoStateLabel.textContent = pomoState === 'focus' ? 'Focus Session' : 'Quick Break Γÿò';
      }
    }
        ...
        pomoInterval = setInterval(() => {
          if (pomoTimeLeft > 0) {
            ...
            if (pomoState === 'focus') {
              ...
              toast('Focus session ended! Time for a cozy tea break. Γÿò', 'success');
            } else {
              pomoState = 'focus';
              pomoTimeLeft = 25 * 60;
              toast('Break complete! Ready to start focus sprint? Γ£¿', 'success');
            }
            updatePomoDisplay();
    ...
    function renderCalEvents() {
      ...
      if (events.length === 0) {
        list.innerHTML =
          '<div class="empty-state"><span class="text-3xl filter saturate-50 select-none">≡ƒîî</span>Nothing on this day</div>';
        return;
      }
    ...
    function gameGuess(direction: 'higher' | 'lower') {
      ...
      if (correct) {
        ...
        if (sub) sub.textContent = 'correct ΓÇö keep going';
        haptic(15);
      } else {
        if (sub) sub.textContent = 'streak broken ΓÇö try again';
        gameStreak = 0;
    ...
    function handleSelectedVideo(file: File) {
      if (!file.type.startsWith('video/')) {
        toast("ΓÜá∩╕Å Only video files are compatible with the Portal background!", "warn");
        return;
      }
      toast("≡ƒö« Channeling video into background database...", "success");
      ...
      saveVideoBlob(file).then(() => {
        toast("Γ£¿ Portal background successfully bound!", "success");
        updateBgControlsUI(true);
      }).catch(err => {
        console.error("IndexedDB Save Error:", err);
        toast("ΓÜá∩╕Å Saved to memory only (storage limit exceeded)", "warn");
        updateBgControlsUI(true);
      });
    }
    ...
    if (clearBgVideoBtn) {
      clearBgVideoBtn.addEventListener('click', () => {
        clearVideoBlob().then(() => {
          ...
          toast("≡ƒîî Reverted background to celestial cosmic nebula", "success");
          updateBgControlsUI(false);
        }).catch(err => {
      ...
      oracleConsultBtn.addEventListener('click', () => {
        ...
        if (oracleResponseShell && oracleSpinner && oracleText) {
          ...
          setTimeout(() => {
            ...
            const index = Math.abs(q.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0)) % oracleAnswers.length;
            oracleText.textContent = `"${q}" ΓåÆ ${oracleAnswers[index]}`;
            haptic([100, 50, 100]);
          }, 1200);
        }
      });
    ...
    if (astrolabeSpinBtn && astrolabeSpinRing && astrolabeSelectedWord) {
      astrolabeSpinBtn.addEventListener('click', () => {
        ...
        setTimeout(() => {
          ...
          isSpinning = false;
          haptic([100, 200]);
          toast(`Astrolabe selects: ${finalWord}! ≡ƒÆ½`, 'success');
        }, 2000);
      });
    }
      ...
      pScores.forEach((p, idx) => {
        ...
        r.innerHTML = `
          ...
          <div class="flex items-center gap-1.5">
            ...
            <button class="w-5 h-6 text-slate-500 hover:text-red-400 font-bold text-[10px] cursor-pointer focus:outline-none" onclick="window.delScore(${idx})">├ù</button>
          </div>
        `;
    ...
    function startSoundscape() {
      try {
        ...
        if (playSoundscapeBtn) playSoundscapeBtn.textContent = '≡ƒöè STOP AMBIENT';
    ...
    function stopSoundscape() {
      ...
      if (playSoundscapeBtn) playSoundscapeBtn.textContent = '≡ƒöç PLAY AMBIENT';
      ...
      magicPasteParseBtn.addEventListener('click', () => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (sectionMode === 'ingredients') {
            ...
            let item = cleanLine.replace(/^[\s\-*ΓÇó+≡ƒºä≡ƒºà≡ƒÑ¢≡ƒºÇ≡ƒÑû≡ƒìù≡ƒÑ⌐≡ƒºé≡ƒì▓≡ƒÑÜ≡ƒì£≡ƒìÜ≡ƒìƒ≡ƒìò≡ƒî¡≡ƒ¢Æ]*\s*/, '').trim();
            item = item.replace(/^\d+[\s\.)\-]\s*/, '').trim(); // "1. ", "2) "
            if (item) detectedIngredients.push(item);
          } else if (sectionMode === 'directions') {
            let step = cleanLine.replace(/^[\s\-*ΓÇó+]*\s*/, '').trim();
            ...
            // Fallback guesses
            if (/^[\-*ΓÇó+]\s*/.test(rawLine)) {
              detectedIngredients.push(cleanLine.replace(/^[\s\-*ΓÇó+]*\s*/, '').trim());
      ...
      if (filtered.length === 0) {
        grid.innerHTML = `
          <div class="card p-6 text-center select-none border border-dashed border-[#44387a]/40 bg-[#150f2e]/20 rounded-2xl">
            <span class="text-3xl block mb-2">≡ƒì╜∩╕Å</span>
      ...
      cookingTimerInterval = setInterval(() => {
        if (cookingTimerSecondsLeft > 0) {
          ...
          if (cookingTimerSecondsLeft === 0) {
            ...
            toast('Cooking Alchemist alert: Cook step is complete! ≡ƒ¢Ä∩╕Å', 'warn');
            flashCookingTimerUi();
            resetCookingTimer();
          }
        } else {
    ...
    function updateFusionSelectionDisplay() {
      ...
      if (checkedIds.length === 0) {
        ...
        if (fuseBtn) {
          ...
          fuseBtn.textContent = 'AUTOCLAVE RECIPES ΓÜù∩╕Å';
        }
        return;
      }
      ...
      selectedRecipes.forEach(sr => {
        ...
        row.innerHTML = `≡ƒº¼ <span class="font-bold truncate">${sr.title}</span>`;
      ...
      if (fuseBtn) {
        if (selectedRecipes.length >= 2) {
          ...
          fuseBtn.textContent = `FUSE ${selectedRecipes.length} CODICES ΓÜù∩╕Å`;
        } else {
          fuseBtn.disabled = true;
    ...
    function completeFusionCreation(selected: any[]) {
      ...
      const titleParts = selected.map(s => s.title.replace(/Viral|TikTok|BabishΓÇÖs|Instagram|Oven|Baked/g, '').trim());
    ...
    function finalizeExtractionResult(linkUrl: string) {
      ...
      if (lower.includes('ramen') || lower.includes('noodle')) {
        ...
        dIngredients = [
          ...
          { name: 'Barbacoa dipping consum├⌐ bouillon', qty: '1 cup', done: false }
        ];
        dDirections = [
          'Dip corn tortillas directly into the warm dipping consum├⌐ fat layer.',
          ...
          'Garnish with onion, cilantro, and squeeze fresh lime. Serve with rich consum├⌐ cups.'
        ];
        ...
        dTitle = `${creatorName}ΓÇÖs Secret ${recipeWord} Delight`;
      }
      ...
      const extractedRecipe = {
        ...
        secretTip: 'Watch the video pacing on the social link directly to replicate the creatorΓÇÖs specific flipping speed!',
    ...
    function executePantrySynthesis() {
      ...
      let synthTitle = 'AlchemistΓÇÖs Custom Stir-Fry Concoction';
      let rDiff = 'Medium';
      let rPrep = '10 mins';
      let rCook = '12 mins';
      ...
      if (items.includes('Eggs') && items.includes('Rice')) {
        ...
        synthTitle = 'Saut├⌐ed Garlic Rosemary Chicken Breast';
        rCook = '15 mins';
      ...
      renderRecipesGrid();
      haptic([70, 100]);
      toast('Dynamic pantry recipe formulated! ≡ƒì│', 'success');
    ...
    let companionMessages = store.get('companion_messages', [
      {
        sender: 'bot',
        text: "Greetings, traveler of the Rift! I am OFFLINE_BOT ΓÇö your personal, fully local, zero-latency companion. I match patterns in what you type and reply from my alchemical scripts.",
        timestamp: Date.now()
      }
    ]);
            ...
            <div class="flex flex-wrap gap-1 mt-1.5 justify-end">
              ${msg.attachments.map((att: any) => `
                <span class="text-[8.5px] px-1.5 py-0.5 rounded bg-white/5 text-[#cf4fe6] border border-[#cf4fe6]/20 font-mono">
                  ${att.type === 'note' ? '≡ƒô¥' : att.type === 'recipe' ? '≡ƒì│' : '≡ƒôî'} ${att.title}
                </span>
              `).join('')}
            </div>
          `;
    ...
    function updateActiveAttachmentsBar() {
      ...
      companionAttachedNotes.forEach((idx) => {
        ...
        tag.innerHTML = `≡ƒô¥ Note: ${n.title || `Note #${idx + 1}`} <button class="ml-1 hover:text-white bg-transparent border-none p-0 cursor-pointer text-amber-400" data-remove-attached-note="${idx}">Γ£ò</button>`;
        bar.appendChild(tag);
      ...
      companionAttachedRecipes.forEach((id) => {
        ...
        tag.innerHTML = `≡ƒì│ Recipe: ${r.title} <button class="ml-1 hover:text-white bg-transparent border-none p-0 cursor-pointer text-pink-400" data-remove-attached-recipe="${id}">Γ£ò</button>`;
        bar.appendChild(tag);
      ...
      companionAttachedTasks.forEach((idx) => {
        ...
        tag.innerHTML = `≡ƒôî Task: ${t.text} <button class="ml-1 hover:text-white bg-transparent border-none p-0 cursor-pointer text-emerald-400" data-remove-attached-task="${idx}">Γ£ò</button>`;
        bar.appendChild(tag);
        ...
        if (mathMatch && /[\+\-\*\/]/.test(mathMatch[1])) {
          try {
            ...
            if (typeof result === 'number' && !isNaN(result)) {
              replyText = `≡ƒö« **Alchemical Arithmetic Alignment**
...
*Computed entirely on-device with zero-latency local arithmetic.*`;
        ...
        if (!replyText && attachments.length > 0) {
          ...
          const parts: string[] = [];
          parts.push("Γ£¿ **LOCAL COGNITIVE ANALYSIS REPORT** Γ£¿\n");

          if (notes.length > 0) {
            parts.push(`≡ƒô¥ **Rift Note Scanned:** *"${notes[0].title}"*`);
            if (notes[0].text) {
          ...
          if (recipes.length > 0) {
            ...
            parts.push(`≡ƒì│ **Alchemical Codex Breakdown:** *"${recipes[0].title}"*`);
          ...
          if (tasks.length > 0) {
            parts.push(`≡ƒôî **Quest Log Inspection:** *"${tasks[0].title}"*`);
          ...
          if (attachments.length > 1) {
            parts.push(`\n≡ƒöù **Cross-Rift Binding:** Coalesced ${attachments.length} separate elements on-device!`);
          }
          ...
          const faqs = [
            {
              ...
              reply: `≡ƒö« **Welcome to your on-device Rift Consciousness!** I run 100% locally inside your browser, completely offline with zero server latency. Here is my capabilities matrix:

1. **Local Calculations:** Type any math expression like \`20 + 10\` or \`15 * 40\` and I will evaluate it instantly!
2. **Data Inspection:** Click the ≡ƒôÄ attach button to bind any **Note**, **Recipe**, or **Quest Task** to our conversation. I will run a local cognitive analysis.
...
4. **Chrome Built-In AI:** If you are running Chrome with \`window.ai\` enabled, I will channel Gemini Nano directly on your GPU without using any internet connection!`
            },
            {
              ...
              reply: "Greetings, traveler of the Rift! I am **OFFLINE_BOT** ΓÇö your personal, fully local companion. No servers, no latency, all running directly on your device."
            },
            {
              keys: ['game', 'cosmic words', 'war', 'rules of cosmic words'],
              reply: `≡ƒö« **Cosmic Words Game Manual:** 
...
- Use the **Game** tab to open the Nexus Chamber and invite friends!`
            },
            {
              ...
              reply: "≡ƒÉë **Dungeons & Dragons Lore:** The dice tray above is ready for action! Cast your D20, manage your spell slots, and use the Notes tab to record your party's epic milestones. Need help with a monster? Attach your encounter notes here!"
            },
            {
              keys: ['spell', 'spellbook', 'slots', 'wizard', 'magic'],
              reply: "Γ£¿ **Spellcasting Codex:** A standard wizard regains spell slots on a Long Rest. Keep track of your level 1-9 spell slots in the Home tab, and enscribe custom scrolls in your Notes!"
            },
            {
              keys: ['joke', 'funny', 'humor'],
              reply: "Why did the wizard refuse to use cloud storage? ...Because they preferred keeping their spell scrolls in a local pocket dimension! ≡ƒîî"
            },
            {
        ...
        if (!replyText) {
          replyText = `≡ƒîî **Offline Rift Companion Core**
...
≡ƒÆí **Try these smart offline commands:**
- Type \`20 + 10\` or any arithmetic expression to calculate values.
- Type \`help\` or \`rules\` to view local capabilities.
- Click the ≡ƒôÄ button below to attach a **Note** or **Recipe** for me to scan.
          ...
          <div className="flex flex-col items-center max-w-sm w-full px-6 text-center select-none z-10 animate-fade-in relative">
            ...
            <div className="w-full portal-glass-card p-6">
              ...
              <p className="text-[10px] text-[#b4aae2]/70 italic tracking-wide mb-4">
                {loginTab === 'login' 
                  ? "≡ƒö« Enter your credentials to awaken your offline data chambers." 
                  : "Γ£¿ Bind a new ID to the localized browser index with a custom 4-digit security PIN."}
              </p>
      ...
      <div id="dragDropVideoOverlay" className="fixed inset-0 bg-[#0d071c]/90 backdrop-blur-md z-[9999] flex flex-col items-center justify-center border-4 border-dashed border-[#cf4fe6] m-4 rounded-3xl opacity-0 pointer-events-none transition-all duration-300">
        <div className="text-center space-y-4 max-w-md p-6 select-none animate-bounce">
          <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-tr from-[#cf4fe6] to-[#3fd9c7] flex items-center justify-center shadow-[0_0_20px_rgba(207,79,230,0.5)]">
            <span className="text-4xl">≡ƒö«</span>
          </div>
      ...
      <main>
          ...
          <div className="home-dashboard flex flex-col gap-5 w-full max-w-5xl mx-auto px-1 py-1 pb-16">
              ...
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#b4aae2] font-mono tracking-widest uppercase mt-0.5">
                    {currentUser === 'trxy6' ? '≡ƒææ SYSTEM CREATOR BOUND TO THE RIFT' : '≡ƒæñ BOUND RIFT TRAVELER'}
                  </p>
                </div>
              </div>
            ...
            {(visibleWidgets.quickLaunch || visibleWidgets.diceTray) && (
              <div className="flex flex-col md:grid md:grid-cols-12 gap-5 w-full">
                ...
                >
                  ...
                  <div className="w-11 h-11 mb-2.5 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                    <span className="text-[32px] filter drop-shadow-[0_0_8px_rgba(255,117,151,0.6)]">≡ƒº¬</span>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-5 items-center justify-between flex-grow py-1">
                  ...
                  <div className="roll-stage relative w-[170px] h-[170px] flex items-center justify-center shrink-0">
                    ...
                    <svg className="absolute inset-0 w-full h-full animate-spin-slow opacity-40 pointer-events-none" viewBox="0 0 200 200">
                      ...
                      <text x="100" y="28" fill="#3fd9c7" fontSize="7" textAnchor="middle" fontFamily="monospace">Γ£╡</text>
                      <text x="100" y="180" fill="#3fd9c7" fontSize="7" textAnchor="middle" fontFamily="monospace">Γ£╡</text>
                      <text x="28" y="103" fill="#cf4fe6" fontSize="7" textAnchor="middle" fontFamily="monospace">ΓÜö</text>
                      <text x="172" y="103" fill="#cf4fe6" fontSize="7" textAnchor="middle" fontFamily="monospace">ΓÜö</text>
                    </svg>
            ...
            {(visibleWidgets.campaignOverview || visibleWidgets.upNext) && (
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5 w-full">
                ...
                <div className="flex justify-between items-center pb-2.5 border-b border-[#2e2454]/60 z-10 mb-4 select-none">
                  <div className="flex items-center gap-2">
                    <span className="text-indigo-400 text-[13px]">Γ£ª</span>
                ...
                <div className="flex items-center gap-3.5 mb-3.5">
                  ...
                  <div className="flex flex-col leading-tight">
                    ...
                    <span className="text-[10px] text-pink-400 mt-0.5 font-bold font-mono uppercase tracking-wider">Main Quest ΓÇö In 3 days</span>
                  </div>
                </div>
              ...
              <div className={`grid grid-cols-1 ${visibleWidgets.oracle && visibleWidgets.habitStreak ? 'md:grid-cols-12' : ''} gap-5 w-full`}>
                ...
                <div>
                  <div className="flex justify-between items-center pb-2.5 border-b border-amber-500/20 mb-3 select-none">
                    <div className="flex items-center gap-1.5">
                      <span className="text-amber-400">≡ƒö«</span>
                ...
                <div>
                  <div className="flex justify-between items-center pb-2.5 border-b border-emerald-500/20 mb-2 select-none">
                    <div className="flex items-center gap-1.5">
                      <span className="text-emerald-400 animate-pulse">≡ƒöÑ</span>
                  ...
                  <div className="flex items-center gap-4 py-2.5 bg-emerald-500/5 border border-emerald-500/20 rounded-xl px-3 mt-1 select-none">
                    <div className="relative flex items-center justify-center shrink-0 w-10 h-10">
                      ...
                      <span className="text-2xl filter drop-shadow-[0_0_6px_#10b981]">≡ƒöÑ</span>
                    </div>
            ...
            {Object.values(visibleWidgets).every(v => v === false) && (
              <div className="flex flex-col items-center justify-center py-16 px-6 text-center border border-dashed border-[#44387a]/45 rounded-3xl bg-[#130a2a]/30 shadow-inner">
                <span className="text-4xl mb-4 filter drop-shadow-[0_0_10px_rgba(207,79,230,0.4)]">Γ£¿</span>
            ...
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
              ...
              <div className="relative w-full max-w-lg rounded-2xl border border-[#cf4fe6]/40 bg-gradient-to-b from-[#1a103c] to-[#0a051d] p-6 shadow-[0_0_50px_rgba(207,79,230,0.15)] flex flex-col max-h-[85vh] overflow-hidden">
                ...
                <div className="flex justify-between items-center pb-4 border-b border-[#3d2766]/60 mb-4 select-none">
                  <div className="flex items-center gap-2">
                    <span className="text-xl">ΓÜÖ∩╕Å</span>
                    <div>
                ...
                <div className="flex-grow overflow-y-auto space-y-2.5 pr-1 py-1">
                  {[
                    { key: 'quickLaunch', label: 'Quick Launch Buttons', desc: 'Shortcuts to Roll Panel, Journal, Stats & Cook', icon: '≡ƒÜÇ' },
                    { key: 'diceTray', label: 'Central Dice Tray', desc: 'Roll dice, flip coins, and view roll history', icon: '≡ƒÄ▓' },
                    { key: 'utilityTiles', label: 'Character & Quest Shortcuts', desc: 'Direct links to characters, quest logs & settings', icon: '≡ƒô£' },
                    { key: 'turnTracker', label: 'Combat Turn Tracker', desc: 'Track rounds, active turns, and initiative', icon: 'ΓÅ│' },
                    { key: 'worldClock', label: 'Astrological Clock & Weather', desc: 'Animated clock dial face and regional weather info', icon: 'ΓÿÇ∩╕Å' },
                    { key: 'campaignOverview', label: 'Campaign Overview Map', desc: 'Detailed campaign settings, player count & calendar', icon: '≡ƒù║∩╕Å' },
                    { key: 'upNext', label: 'Up Next Active Quest', desc: 'Track current milestones and quick task checkboxes', icon: '≡ƒÄ»' },
                    { key: 'oracle', label: 'Fate Decision Oracle', desc: 'Consult the cosmos and ask open-ended questions', icon: '≡ƒö«' },
                    { key: 'habitStreak', label: 'Quest Habit Streak', desc: 'Keep track of consecutive days completing routines', icon: '≡ƒöÑ' },
                  ].map((w) => (
                    <div 
                      key={w.key}
        ...
        <div className="panel" id="panel-roll">
          <div className="sub-navigation">
            <button className="sub-nav-btn" data-sub-panel="sheet" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('sheet'); }}>≡ƒô£ Hero Sheet</button>
            <button className="sub-nav-btn" data-sub-panel="roll" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('roll'); }}>≡ƒÄ▓ Dice Basin</button>
            <button className="sub-nav-btn" data-sub-panel="recipes" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('recipes'); }}>≡ƒì│ Camp Cook</button>
            <button className="sub-nav-btn" data-sub-panel="game" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('game'); }}>≡ƒÄ« Games Chamber</button>
          </div>
          ...
          <div className="sub-navigation flex flex-col md:flex-row gap-2.5 items-stretch md:items-center justify-between border-b border-[#2e2454]/45 pb-3 mb-4 select-none">
            ...
            <div className="flex items-center bg-[#150f2e]/60 rounded-xl border border-[#44387a]/45 p-0.5 w-full md:w-auto">
              ...
              <div className="flex items-center gap-0.5 flex-1 md:flex-initial">
                <button className="sub-nav-btn flex-grow md:flex-initial py-1 px-2.5 rounded-lg text-[10.5px] font-bold transition-all duration-150 cursor-pointer" data-sub-panel="notes" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('notes'); }}>≡ƒô¥ Notes</button>
                <button className="sub-nav-btn flex-grow md:flex-initial py-1 px-2.5 rounded-lg text-[10.5px] font-bold transition-all duration-150 cursor-pointer" data-sub-panel="calendar" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('calendar'); }}>≡ƒôà Calendar</button>
                <button className="sub-nav-btn flex-grow md:flex-initial py-1 px-2.5 rounded-lg text-[10.5px] font-bold transition-all duration-150 cursor-pointer" data-sub-panel="tasks" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('tasks'); }}>Γ£à Tasks</button>
              </div>
            </div>
            {/* TOOLS GROUP */}
            <div className="flex items-center bg-[#150f2e]/60 rounded-xl border border-[#44387a]/45 p-0.5 w-full md:w-auto">
              ...
              <div className="flex items-center gap-0.5 flex-1 md:flex-initial">
                <button className="sub-nav-btn flex-grow md:flex-initial py-1 px-2.5 rounded-lg text-[10.5px] font-bold transition-all duration-150 cursor-pointer" data-sub-panel="calc" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('calc'); }}>≡ƒº« Calc</button>
                <button className="sub-nav-btn flex-grow md:flex-initial py-1 px-2.5 rounded-lg text-[10.5px] font-bold transition-all duration-150 cursor-pointer" data-sub-panel="clock" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('clock'); }}>ΓÅ│ Timer</button>
                <button className="sub-nav-btn flex-grow md:flex-initial py-1 px-2.5 rounded-lg text-[10.5px] font-bold transition-all duration-150 cursor-pointer" data-sub-panel="sports" onClick={() => { if ((window as any).switchToPanel) (window as any).switchToPanel('sports'); }}>≡ƒÅå Sports</button>
              </div>
            </div>
          </div>
          ...
          <section className="card">
            ...
            <div className="calc-sub active" id="calc-basic">
              ...
              <div className="calc-keys" id="calcKeys">
                {['C', '┬▒', '%', '├╖', '7', '8', '9', '├ù', '4', '5', '6', '-', '1', '2', '3', '+', '0', '.', 'Γî½', '='].map((k) => (
                  <button
                    key={k}
                    className={`calc-key${['├╖', '├ù', '-', '+'].includes(k) ? ' op' : ''}${k === '=' ? ' eq' : ''}${['C', '┬▒', '%', 'Γî½'].includes(k) ? ' fn' : ''}`}
                    onClick={() => {
            ...
            <div className="calc-sub" id="calc-ti84">
              <div className="w-full flex flex-col items-center justify-center p-2 bg-[#0b061d]/60 rounded-3xl border border-purple-500/20 relative overflow-hidden">
                {/* TI-84 Shell Container with cosmic galaxy styling */}
                <div className="w-full max-w-[430px] galaxy-shell rounded-[40px] p-4.5 pt-6 pb-6 flex flex-col relative overflow-hidden">
                  
                  {/* Texas Instruments Top Branding Banner */}
                  <div className="flex justify-between items-center px-3 mb-2.5 z-10">
                    <span className="text-[9px] uppercase tracking-widest text-purple-300/60 font-black">Texas Instruments</span>
                    <span className="text-xs font-black text-white tracking-tight flex items-center gap-1.5">
                      TI-84 Plus CE <span className="text-[#3fd9c7] text-[8px] tracking-widest bg-[#3fd9c7]/10 px-1.5 py-0.5 rounded-full border border-[#3fd9c7]/20 uppercase">Cosmic OS</span>
                    </span>
                  </div>

                  {/* Calculator Bezel screen space styled with galaxy elements */}
                  <div className="galaxy-bezel p-2.5 rounded-2xl mb-4.5 shadow-inner relative overflow-hidden">
                    
                    {/* Twinkling star decorations inside bezel for authentic galaxy feel */}
                    <div className="galaxy-star" style={{ top: '8%', left: '12%', width: '2px', height: '2px', '--duration': '2.2s' } as any} />
                    <div className="galaxy-star" style={{ top: '15%', right: '15%', width: '1.5px', height: '1.5px', '--duration': '3.1s' } as any} />
                    <div className="galaxy-star" style={{ top: '4%', right: '40%', width: '2.5px', height: '2.5px', '--duration': '1.8s' } as any} />
                    <div className="galaxy-star" style={{ bottom: '10%', left: '22%', width: '1.5px', height: '1.5px', '--duration': '2.7s' } as any} />
                    <div className="galaxy-star" style={{ bottom: '6%', right: '28%', width: '2px', height: '2px', '--duration': '3.5s' } as any} />
                    
                    {/* Status Bar Header */}
                    <div className="flex justify-between items-center text-[9px] font-mono font-bold text-[#3fd9c7] border-b border-purple-500/10 pb-1.5 mb-2 z-10 relative">
                    ...
                    <div ref={lcdScreenRef} className="h-44 galaxy-screen p-2.5 font-mono text-xs overflow-y-auto rounded-lg lcd-glow flex flex-col relative select-none border border-[#3fd9c7]/10 z-10">
                      
                      {/* Dynamic OS Bootloader fallback to guard MathJS installation */}
                      {!mathLoaded ? (
                        <div className="flex-1 flex flex-col items-center justify-center font-mono p-2">
                          <div className="text-center space-y-2.5">
                            ...
                            <div className="w-24 h-2.5 border border-purple-500/40 rounded-sm p-[1px] mt-2.5 mx-auto bg-transparent">
                              <div className="h-full bg-gradient-to-r from-[#3fd9c7] to-[#cf4fe6] animate-loading-bar rounded-[1px]" style={{ width: '70%' }}></div>
                            </div>
                            <div className="text-[8px] mt-2 opacity-75 font-semibold text-[#b4aae2]/60">STAR DUST CALIBRATED ΓÇó OK</div>
                          </div>
                        </div>
                      ) : (
                        <>
                          ...
                          {currentScreen === 'HOME' && (
                            <div className="flex-1 flex flex-col justify-end">
                              ...
                              <div className="overflow-y-auto space-y-1.5 max-h-28 flex-1 pr-0.5">
                                {history.map((h, i) => (
                              ...
                              {/* Active calculation input line with blinking block cursor */}
                              <div className="border-t border-purple-500/10 pt-1 mt-1 flex items-center">
                                ...
                                <span className="relative inline-block break-all max-w-full font-bold text-white">
                                  ...
                                  <span className="bg-[#3fd9c7] text-[#020008] cursor-blink inline-block w-1.5 h-3 text-center">
                              ...
                              <div className="space-y-1 flex-1">
                                {['Y1', 'Y2', 'Y3', 'Y4'].map((eqKey) => (
                                  ...
                                  >
                                    ...
                                    <span className="flex-1 ml-2 font-bold font-mono text-white truncate max-w-[170px]">
                            ...
                            <div className="flex-1 flex flex-col justify-between">
                              <div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  >
                                    <span className="font-bold text-[9px] text-[#b4aae2]">╬öTbl = </span>
                          ...
                          {currentScreen === 'GRAPH' && (
                            <div className="flex-1 flex flex-col relative rounded-md overflow-hidden border border-purple-500/10">
                              <canvas 
                                ref={canvasRef} 
                                width={380} 
                                height={210} 
                                className="w-full h-full flex-1 bg-[#100b26] cursor-crosshair"
                              />
                          ...
                          {currentScreen === 'TABLE' && (
                            <div className="flex-1 flex flex-col">
                              <div className="grid grid-cols-3 font-black text-[9px] tracking-wider text-purple-300 border-b border-purple-500/20 pb-1 mb-1 text-center">
                                <span className="text-left pl-2">X</span>
                                <span>Y1</span>
                                <span>Y2</span>
                              </div>
                              <div className="flex-1 overflow-y-hidden divide-y divide-purple-500/5 text-center">
                                {renderTableRows().map((row, i) => (
                                  <div key={i} className="grid grid-cols-3 text-[9px] py-1 font-mono font-bold">
                          ...
                          {currentScreen === 'MODE_MENU' && (
                            <div className="flex-1 flex flex-col text-[9px] space-y-1.5 overflow-y-auto">
                          ...
                          {currentScreen === 'MATH_MENU' && (
                            <div className="flex-1 flex flex-col">
                              ...
                              <div className="space-y-1 pt-0.5">
                                {[
                                  { l: '1: Γû╢Frac', d: 'Convert to fraction', t: 'toFraction(' },
                                  { l: '2: Γû╢Dec', d: 'Convert to decimal', t: 'string(' },
                  ...
                  <div className="grid grid-cols-5 gap-1.5 select-none mt-2">
                    ...
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Stat Plot</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">Frac</span>
                      <button onClick={() => { haptic(10); handleButtonPress('Y='); }} className="w-full py-1.5 galaxy-btn-blue rounded text-[9px] font-bold cursor-pointer transition-all">Y=</button>
                    </div>
                    
                    {/* GRAPH */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Window</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">Func</span>
                      <button onClick={() => { haptic(10); handleButtonPress('GRAPH'); }} className="w-full py-1.5 galaxy-btn-blue rounded text-[9px] font-bold cursor-pointer transition-all">GRAPH</button>
                    </div>

                    {/* Home */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Format</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">Mtrx</span>
                      <button onClick={() => { haptic(10); handleButtonPress('QUIT'); }} className="w-full py-1.5 galaxy-btn-blue rounded text-[9px] font-bold cursor-pointer transition-all">Home</button>
                    </div>

                    {/* TABLE */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">TblSet</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">Yvar</span>
                      <button onClick={() => { haptic(10); handleButtonPress('TABLE'); }} className="w-full py-1.5 galaxy-btn-blue rounded text-[9px] font-bold cursor-pointer transition-all">TABLE</button>
                    </div>

                    {/* Undo */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Redo</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">Conv</span>
                      <button onClick={() => { haptic(10); setInputVal(''); setCursorIndex(0); }} className="w-full py-1.5 galaxy-btn-blue rounded text-[9px] font-bold cursor-pointer transition-all">Undo</button>
                    </div>
                    ...
                    <div className="flex flex-col items-center col-span-1">
                      ...
                      <span className="text-[7px] font-bold opacity-0">.</span>
                      <button onClick={() => { haptic(10); handleButtonPress('2ND'); }} className="w-full py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded text-[9px] font-black uppercase cursor-pointer transition-all shadow-[0_0_8px_rgba(245,158,11,0.3)]">2nd</button>
                    </div>

                    {/* MODE */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Quit</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">User</span>
                      <button onClick={() => { haptic(10); handleButtonPress('MODE'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">MODE</button>
                    </div>

                    {/* DEL */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Ins</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">uKeys</span>
                      <button onClick={() => { haptic(10); handleButtonPress('DEL'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">DEL</button>
                    </div>

                    {/* D-PAD Container (Occupies columns 4 and 5, rows 2 and 3) */}
                    <div className="col-span-2 row-span-2 flex items-center justify-center bg-[#070414]/90 border border-purple-500/20 rounded-2xl p-1.5">
                      <div className="grid grid-cols-3 grid-rows-3 gap-0.5 w-full h-full max-w-[120px] max-h-[80px]">
                        <div />
                        <button onClick={() => { haptic(10); handleButtonPress('UP'); }} className="bg-[#1b2650]/80 hover:bg-[#2b3a6c]/90 text-white rounded-lg flex flex-col items-center justify-center p-0.5 cursor-pointer border border-[#3b82f6]/20">
                          <span className="text-[6px] galaxy-text-cyan font-bold leading-none">A+</span>
                          <span className="text-[9px] leading-none">Γû▓</span>
                        </button>
                        <div />
                        <button onClick={() => { haptic(10); handleButtonPress('LEFT'); }} className="bg-[#1b2650]/80 hover:bg-[#2b3a6c]/90 text-white rounded-lg flex items-center justify-center text-[9px] cursor-pointer border border-[#3b82f6]/20">ΓùÇ</button>
                        <div className="bg-[#cf4fe6]/20 rounded-full w-2 h-2 m-auto animate-pulse" />
                        <button onClick={() => { haptic(10); handleButtonPress('RIGHT'); }} className="bg-[#1b2650]/80 hover:bg-[#2b3a6c]/90 text-white rounded-lg flex items-center justify-center text-[9px] cursor-pointer border border-[#3b82f6]/20">Γû╢</button>
                        <div />
                        <button onClick={() => { haptic(10); handleButtonPress('DOWN'); }} className="bg-[#1b2650]/80 hover:bg-[#2b3a6c]/90 text-white rounded-lg flex flex-col items-center justify-center p-0.5 cursor-pointer border border-[#3b82f6]/20">
                          <span className="text-[9px] leading-none">Γû╝</span>
                          <span className="text-[6px] galaxy-text-cyan font-bold leading-none">A-</span>
                        </button>
                        <div />
                    ...
                    {/* ALPHA */}
                    <div className="flex flex-col items-center col-span-1">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">A-Lock</span>
                      <span className="text-[7px] font-bold opacity-0">.</span>
                      <button onClick={() => { haptic(10); handleButtonPress('ALPHA'); }} className="w-full py-1.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded text-[9px] font-black cursor-pointer transition-all shadow-[0_0_8px_rgba(16,185,129,0.3)]">ALPHA</button>
                    </div>

                    {/* X */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Share</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">n/d</span>
                      <button onClick={() => { haptic(10); handleButtonPress('x'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">X</button>
                    </div>

                    {/* STAT */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">List</span>
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">0h</span>
                      <button onClick={() => { haptic(10); handleButtonPress('PRGM'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">STAT</button>
                    </div>

                    {/* ROW 4: 5 buttons */}
                    {/* MATH */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Test</span>
                      <button onClick={() => { haptic(10); handleButtonPress('MATH'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">MATH</button>
                    </div>
                    {/* APPS */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Angle</span>
                      <button onClick={() => { haptic(10); handleButtonPress('MODE'); }} className="w-full py-1.5 galaxy-btn-blue rounded text-[9px] font-bold cursor-pointer transition-all">APPS</button>
                    </div>
                    {/* PRGM */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Draw</span>
                      <button onClick={() => { haptic(10); handleButtonPress('PRGM'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">PRGM</button>
                    </div>
                    {/* VARS */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Distr</span>
                      <button onClick={() => { haptic(10); handleButtonPress('MATH'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">VARS</button>
                    </div>
                    {/* CLEAR */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">ClrDraw</span>
                      <button onClick={() => { haptic(10); handleButtonPress('CLEAR'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[9px] font-bold cursor-pointer transition-all">CLEAR</button>
                    </div>

                    {/* ROW 5: 5 buttons */}
                    {/* x^-1 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Matrix</span>
                      <button onClick={() => { haptic(10); insertToken('^-1'); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">xΓü╗┬╣</button>
                    </div>
                    {/* SIN */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">SinΓü╗┬╣</span>
                      <button onClick={() => { haptic(10); handleButtonPress('sin(', 'asin('); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">SIN</button>
                    </div>
                    {/* COS */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">CosΓü╗┬╣</span>
                      <button onClick={() => { haptic(10); handleButtonPress('cos(', 'acos('); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">COS</button>
                    </div>
                    {/* TAN */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">TanΓü╗┬╣</span>
                      <button onClick={() => { haptic(10); handleButtonPress('tan(', 'atan('); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">TAN</button>
                    </div>
                    {/* ^ */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">╧Ç</span>
                      <button onClick={() => { haptic(10); handleButtonPress('^', '╧Ç'); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">^</button>
                    </div>

                    {/* ROW 6: 5 buttons */}
                    {/* x^2 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">ΓêÜ</span>
                      <button onClick={() => { haptic(10); handleButtonPress('┬▓', 'ΓêÜ('); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">x┬▓</button>
                    </div>
                    {/* , */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">EE</span>
                      <button onClick={() => { haptic(10); insertToken(','); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">,</button>
                    </div>
                    {/* ( */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">{"{"}</span>
                      <button onClick={() => { haptic(10); insertToken('('); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">(</button>
                    </div>
                    {/* ) */}
                    <div className="flex flex-col items-center">
                    ...
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] font-bold opacity-0">.</span>
                      <button onClick={() => { haptic(10); handleButtonPress('├╖'); }} className="w-full py-1.5 galaxy-btn-operator rounded text-[10px] font-bold cursor-pointer transition-all">├╖</button>
                    </div>

                    {/* ROW 7: 5 buttons */}
                    {/* LOG */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">10^x</span>
                      <button onClick={() => { haptic(10); handleButtonPress('log(', '10^'); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">LOG</button>
                    </div>
                    {/* 7 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">u</span>
                      <button onClick={() => { haptic(10); insertToken('7'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">7</button>
                    </div>
                    {/* 8 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">v</span>
                      <button onClick={() => { haptic(10); insertToken('8'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">8</button>
                    </div>
                    {/* 9 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">w</span>
                      <button onClick={() => { haptic(10); insertToken('9'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">9</button>
                    </div>
                    {/* ├ù */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">[</span>
                      <button onClick={() => { haptic(10); handleButtonPress('├ù'); }} className="w-full py-1.5 galaxy-btn-operator rounded text-[10px] font-bold cursor-pointer transition-all">├ù</button>
                    </div>

                    {/* ROW 8: 5 buttons */}
                    {/* LN */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">e^x</span>
                      <button onClick={() => { haptic(10); handleButtonPress('ln(', 'e^'); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">LN</button>
                    </div>
                    {/* 4 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">L4</span>
                      <button onClick={() => { haptic(10); insertToken('4'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">4</button>
                    </div>
                    {/* 5 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">L5</span>
                      <button onClick={() => { haptic(10); insertToken('5'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">5</button>
                    </div>
                    {/* 6 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">L6</span>
                      <button onClick={() => { haptic(10); insertToken('6'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">6</button>
                    </div>
                    {/* - */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">]</span>
                    ...
                    {/* STOΓ₧ö */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Rcl</span>
                      <button onClick={() => { haptic(10); insertToken('Γ₧ö'); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] font-mono cursor-pointer transition-all">STOΓ₧ö</button>
                    </div>
                    {/* 1 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">L1</span>
                      <button onClick={() => { haptic(10); insertToken('1'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">1</button>
                    </div>
                    {/* 2 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">L2</span>
                      <button onClick={() => { haptic(10); insertToken('2'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">2</button>
                    </div>
                    {/* 3 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">L3</span>
                      <button onClick={() => { haptic(10); insertToken('3'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">3</button>
                    </div>
                    {/* + */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Mem</span>
                      <button onClick={() => { haptic(10); handleButtonPress('+'); }} className="w-full py-1.5 galaxy-btn-operator rounded text-[10px] font-bold cursor-pointer transition-all">+</button>
                    </div>
                    ...
                    {/* ? (info) */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Γôÿ</span>
                      <button onClick={() => { haptic(10); setCurrentScreen('CATALOG'); }} className="w-full py-1.5 galaxy-btn-black rounded text-[9px] flex items-center justify-center cursor-pointer transition-all">
                        <span className="bg-[#ffb703] text-[#1a1438] text-[8px] font-bold rounded-full w-3.5 h-3.5 flex items-center justify-center">?</span>
                      </button>
                    </div>
                    {/* 0 */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Catalog</span>
                      <button onClick={() => { haptic(10); insertToken('0'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">0</button>
                    </div>
                    {/* . */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-cyan font-bold uppercase">i</span>
                      <button onClick={() => { haptic(10); insertToken('.'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">.</button>
                    </div>
                    {/* (-) */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Ans</span>
                      <button onClick={() => { haptic(10); handleButtonPress('(-)', 'Ans'); }} className="w-full py-1.5 galaxy-btn-white rounded text-[10px] font-bold cursor-pointer transition-all">(-)</button>
                    </div>
                    {/* ENTER */}
                    <div className="flex flex-col items-center">
                      <span className="text-[7px] galaxy-text-yellow font-bold uppercase">Entry</span>
                      <button onClick={() => { haptic(10); handleButtonPress('ENTER'); }} className="w-full py-1.5 bg-[#4f46e5] hover:bg-[#4338ca] text-white rounded text-[9px] font-extrabold uppercase cursor-pointer transition-all shadow-[0_0_8px_rgba(79,70,229,0.4)]">ENTER</button>
                    </div>
          ...
          <section className="card">
            ...
            <div className="task-input-row flex gap-2">
              ...
              <select id="taskPrioritySelect" className="bg-[#1a1138] border border-[#44387a]/40 rounded-lg px-2 text-xs text-[#b4aae2] focus:outline-none focus:border-[#cf4fe6] cursor-pointer">
                <option value="critical">≡ƒÜ¿ Lethal</option>
                <option value="main">≡ƒÄ» Main</option>
                <option value="minor" selected>ΓÜÖ Minor</option>
                <option value="routine">≡ƒöä Routine</option>
              </select>
          ...
          <section className="card">
            ...
            <div className="calc-sub" id="clock-alarm">
              ...
              <p className="settings-note">
                ...
                app can ΓÇö that requires system-level access a web app cannot get.
              </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 w-full">
            ...
            <div className="col-span-12 lg:col-span-8 flex flex-col gap-4">
              <section className="card p-5 rounded-2xl border border-[#44387a]/60 bg-gradient-to-b from-[#160f2e] to-[#080214] shadow-[0_10px_35px_rgba(0,0,0,0.5)]">
                <div className="flex justify-between items-center pb-2.5 border-b border-[#2e2454]/60 mb-4 select-none">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[#3fd9c7]">≡ƒÅå</span>
                ...
                <div className="mb-4 p-3 bg-teal-500/5 border border-teal-500/20 rounded-xl flex items-start gap-2.5">
                  <span className="text-teal-400 mt-0.5 text-xs">≡ƒîÉ</span>
                  <div>
                              ...
                              return (
                                <div key={competitor.id} className="flex items-center justify-between">
                                  <div className="flex items-center gap-2 max-w-[70%]">
                                    ...
                                    >
                                      {isTeamInSlip(competitor.id) ? 'Γ£ô' : '+'}
                                    </button>
                                    {logoUrl ? (
            ...
            <div className="col-span-12 lg:col-span-4 flex flex-col gap-4">
              <section className="card p-5 rounded-2xl border border-[#44387a]/60 bg-gradient-to-b from-[#160f2e] to-[#080214] shadow-[0_10px_35px_rgba(0,0,0,0.5)]">
                <div className="flex justify-between items-center pb-2.5 border-b border-[#2e2454]/60 mb-3 select-none">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[#ff7597]">≡ƒôî</span>
                  ...
                  {sportsFavorites.length === 0 ? (
                    ...
                    sportsFavorites.map((team, index) => (
                      <span className="inline-flex items-center gap-1.5 border border-teal-500/30 bg-teal-500/5 text-white rounded-lg px-2 py-1 text-[10.5px] font-mono" key={team}>
                        <span>{team}</span>
                        <button className="text-red-400 hover:text-red-300 ml-1 font-bold cursor-pointer" onClick={() => removeSportsFavorite(index)}>├ù</button>
                      </span>
                    ))
                  )}
                </div>
              ...
              <section className="card p-5 rounded-2xl border border-purple-500/30 bg-purple-950/5 backdrop-blur-md shadow-[0_0_30px_rgba(168,85,247,0.12)]">
                <div className="flex justify-between items-center pb-2.5 border-b border-purple-500/20 mb-3 select-none">
                  <div className="flex items-center gap-1.5">
                    <span className="text-purple-400">≡ƒö«</span>
                          ...
                          <div className="flex justify-between items-center select-none">
                            ...
                            <div className="flex items-center gap-1.5">
                              <span className={`text-[8.5px] font-extrabold px-2 py-0.5 rounded-full border uppercase tracking-wider ${
                                ...
                                {parlay.status === 'won' ? 'WON ≡ƒÅå' : parlay.status === 'lost' ? 'LOST Γ¥î' : parlay.status === 'live' ? 'LIVE ≡ƒÄ«' : 'PENDING ΓÅ│'}
                              </span>
              ...
              <section className="card p-4 rounded-2xl border border-[#44387a]/45 bg-[#0f0a28]/60 select-none">
                <h4 className="text-[10.5px] font-bold text-[#faebd7] uppercase tracking-wider mb-2 flex items-center gap-1">
                  <span>≡ƒÆí</span> Why is it 100% Free?
                </h4>
                <ul className="space-y-1.5 text-[9.5px] text-[#b4aae2]/80 leading-normal">
                  <li className="flex items-start gap-1">
                    <span className="text-emerald-400 shrink-0">Γ£ô</span>
        ...
        <div className="panel" id="panel-recipes">
          ...
          <section className="card p-4 relative overflow-hidden mb-4">
            ...
            <div className="flex justify-between items-center pb-2 border-b border-[#2e2454]/60 mb-3 select-none">
              <div className="flex items-center gap-2">
                <span className="text-[#ff7597] text-sm animate-pulse">≡ƒº¬</span>
              ...
              <div id="manualRecipeForm" className="hidden flex flex-col gap-3">
                ...
                <div className="bg-[#1b1236]/80 p-3 rounded-xl border border-[#44387a]/60 flex flex-col gap-2">
                  <div className="flex items-center gap-1.5 justify-between">
                    <span className="text-[8px] font-bold text-[#ff7597] tracking-[0.18em] uppercase block select-none">Γ£¿ Magic Clipboard Paste (AI / Gemini / Blogs)</span>
          ...
          <div className="recipe-tab-content hidden" id="recipeTab-fusion">
            <section className="card p-4 text-center">
              <div className="text-3xl mb-1.5">ΓÜù∩╕Å</div>
            ...
            <div className="flex-grow overflow-y-auto p-5 space-y-5">
              ...
              <div className="bg-[#120a24]/90 border border-amber-500/30 p-4 rounded-3xl relative overflow-hidden">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="bg-amber-500/10 p-2 border border-amber-500/20 rounded-full text-amber-400 text-lg animate-pulse">ΓÅ░</div>
        ...
        <div className="panel animate-fade-in" id="panel-companion">
          <section className="magic-card-frame p-4.5 relative overflow-hidden mb-4 flex flex-col h-[74vh] sm:h-[78vh]">
            ...
            <div className="flex justify-between items-center pb-2.5 border-b border-[#2e2454]/60 mb-3 select-none z-10">
              ...
              <span className="text-[8.5px] text-[#cf4fe6] font-bold border border-[#cf4fe6]/30 px-2.5 py-0.5 rounded-full bg-[#cf4fe6]/10 uppercase tracking-widest font-mono">ΓùÅ LOCAL ONLY</span>
            </div>
            ...
            <div className="flex items-center gap-2 mt-3 pt-2.5 border-t border-[#2e2454]/60 relative">
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2.5 text-left h-[130px] overflow-y-auto pr-1">
                  {/* Notes Column */}
                  <div className="flex flex-col">
                    <span className="text-[8.5px] font-extrabold text-amber-400 border-b border-amber-500/10 pb-0.5 mb-1 uppercase tracking-wide">≡ƒô¥ Notes</span>
                  ...
                  {/* Recipes Column */}
                  <div className="flex flex-col">
                    <span className="text-[8.5px] font-extrabold text-pink-400 border-b border-pink-500/10 pb-0.5 mb-1 uppercase tracking-wide">≡ƒì│ Recipes</span>
                  ...
                  <div className="flex flex-col">
                    <span className="text-[8.5px] font-extrabold text-emerald-400 border-b border-emerald-500/10 pb-0.5 mb-1 uppercase tracking-wide">≡ƒôî Quests</span>
        ...
        <div className="panel" id="panel-settings">
          {/* PORTAL SESSION CARD */}
          <section className="card">
            <p className="section-label text-[#cf4fe6]">≡ƒö« Portal Traveler Rift</p>
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-[#b4aae2] font-mono tracking-wider uppercase mt-0.5">
                    {currentUser === 'trxy6' ? '≡ƒææ Creator (trxy6)' : `≡ƒæñ Traveler (${currentUser})`}
                  </p>
                </div>
                <button
                  onClick={() => {
                  ...
                  <div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        toast("≡ƒôï Live link copied to clipboard!", "success");
                        haptic(15);
                      }}
                    ...
                    <div className="flex gap-2 justify-center sm:justify-start">
                      <button
                        onClick={() => {
                          ...
                          toast("≡ƒôï GitHub link copied!", "success");
                          haptic(15);
                        }}
          ...
          {currentUser === 'trxy6' && (
            <section className="card border border-[#cf4fe6]/30">
              <div className="flex justify-between items-center pb-2 border-b border-[#cf4fe6]/15 mb-3">
                <p className="section-label text-[#cf4fe6] m-0">≡ƒô¼ FEEDBACK IDEAS FROM TRAVELERS</p>
                <button
                  onClick={() => {
                    haptic(30);
                    if (confirm("Archive all feature ideas from the rift?")) {
                      ...
                      fetch('/api/feedback/clear', { method: 'POST' })
                        .then(() => {
                          toast("≡ƒô¼ Ideas archived successfully!", "success");
                        })
                        .catch(() => {
                          toast("≡ƒô¼ Ideas archived successfully!", "success");
                        });
          ...
          {currentUser !== 'trxy6' && (
            <section className="card border border-[#3fd9c7]/30">
              <p className="section-label text-[#3fd9c7]">≡ƒîî Channel Feature Idea to Creator</p>
              <p className="settings-note mb-3">
                ...
                <button
                  onClick={() => {
                    ...
                    try {
                      ...
                      // Write to trxy6_secure_terminal_db
                      try {
                        const dbv = localStorage.getItem('trxy6_secure_terminal_db');
                      ...
                      fetch('/api/feedback', {
                        ...
                        if (res.ok) {
                          toast("Γ£¿ Idea channeled directly to trxy6! Thank you!", "success");
                        } else {
                          toast("Γ£¿ Idea saved locally (offline mode).", "success");
                        }
                      }).catch(() => {
                        toast("Γ£¿ Idea saved locally (offline mode).", "success");
                      });
                      ...
                      toast("ΓÜá∩╕Å Rift signal disrupted. Try again.", "error");
                    }
                  }}
          ...
          <>
              ...
              <section className="card">
                ...
                <div className="setting-row flex items-center justify-between gap-3 mb-3">
                  ...
                  <select id="soundscapeSelect" className="bg-[#1a1138] border border-[#44387a]/40 rounded-lg px-2.5 py-1.5 text-xs text-[#b4aae2] focus:outline-none focus:border-[#cf4fe6] w-40">
                    <option value="space">≡ƒîî Deep Space Hum</option>
                    <option value="campfire">≡ƒöÑ Cozy Campfire</option>
                    <option value="chimes">≡ƒÄÉ Astral Chimes</option>
                  </select>
                </div>
              ...
              <section className="card">
                ...
                <p className="settings-note">
                  Everything you enter ΓÇö notes, sheet, tasks, roll history ΓÇö stays only
              ...
              <section className="card">
                ...
                <p className="settings-note">
                  The Portal ΓÇö a companion for the table and beyond. Roll fair dice,
          ...
          <div className="relative w-full max-w-2xl bg-[#140a24]/90 border-2 border-[#3fd9c7]/50 rounded-2xl p-5 shadow-[0_0_35px_rgba(63,217,199,0.3)] flex flex-col gap-4 animate-fade-in my-auto">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-1 bg-[#120826]/80 p-1 rounded-xl border border-white/5">
              {(['notes', 'betslip', 'character', 'dice', 'calendar', 'ask'] as const).map((mode) => (
                ...
                >
                  {mode === 'notes' ? '≡ƒô¥ Notes' : mode === 'betslip' ? '≡ƒÄ½ Slip' : mode === 'character' ? '≡ƒô£ Sheet' : mode === 'dice' ? '≡ƒÄ▓ Dice' : mode === 'calendar' ? '≡ƒôà Agenda' : '≡ƒÆ¼ Ask'}
                </button>
              ))}
            </div>
