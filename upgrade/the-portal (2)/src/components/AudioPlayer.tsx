...
export function AudioPlayer({ themeColor, spotifyToken }: AudioPlayerProps) {
  ...
  return (
    <div id="portal-radio" className="flex flex-col md:flex-row items-center justify-between w-full h-full px-2 sm:px-4 gap-3 md:gap-4">
        ...
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          {spotifyTrack && spotifyTrack.coverArt ? (
            <img 
              ...
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg border object-cover shrink-0 select-none"
              style={{ 
            ...
            <div 
              className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-slate-900 border flex items-center justify-center overflow-hidden shrink-0"
              style={{ borderColor: getThemeHex() + '50' }}
            >
              {/* Breathing orbit inside disc */}
              <div 
                className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border border-dashed animate-[spin_8s_linear_infinite] flex items-center justify-center`}
                style={{ borderColor: getThemeHex() }}
              >
                <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full" style={{ backgroundColor: getThemeHex() }} />
              </div>
              {isCurrentlyPlaying && (
          ...
          <div className="flex flex-col text-left min-w-0">
            <div className="text-[10.5px] sm:text-xs font-bold text-slate-100 uppercase tracking-wider truncate max-w-[130px] sm:max-w-[200px]">
              {spotifyTrack ? spotifyTrack.name : TRACKS[currentTrackIndex].name}
            </div>
            <div className="text-[9px] sm:text-[10px] text-slate-500 font-medium whitespace-nowrap truncate max-w-[110px] sm:max-w-[180px]">
              {spotifyTrack ? spotifyTrack.artist : `Portal Radio • ${TRACKS[currentTrackIndex].genre}`}
            </div>
          </div>
        </div>

        {/* Playback Buttons grouped alongside track info on mobile */}
        <div className="flex items-center gap-1.5 md:hidden shrink-0">
          <button 
            id="radio-prev-btn-mob"
            onClick={handlePrev} 
          ...
          <button 
            id="radio-play-btn-mob"
            onClick={togglePlay} 
            className="p-2 rounded-full bg-slate-900 border text-slate-100 hover:scale-105 active:scale-95 transition-all shadow-md relative overflow-hidden cursor-pointer"
            style={{ borderColor: getThemeHex() }}
          >
            {isCurrentlyPlaying ? (
              <Pause className="w-3.5 h-3.5" />
            ) : (
              <Play className="w-3.5 h-3.5 fill-slate-100 translate-x-[0.5px]" />
            )}
          </button>
          
          <button 
            id="radio-next-btn-mob"
            onClick={handleNext} 
            className="p-1 text-slate-400 hover:text-slate-100 transition-colors cursor-pointer active:scale-90"
          >
      ...
      <div className="flex flex-col items-center gap-1 w-full md:max-w-md">
        ...
        <div className="flex items-center gap-1.5 w-full text-[8px] sm:text-[9px] font-mono text-slate-500">
          <span className="shrink-0">{spotifyTrack ? formatTime(spotifyTrack.progressMs) : `0:${String(Math.floor(progress * 1.8)).padStart(2, '0')}`}</span>
          <div className="relative flex-1 h-1 bg-slate-900 border border-slate-800/50 rounded-full overflow-hidden">
            <div 
          ...
          <span className="shrink-0">{spotifyTrack ? formatTime(spotifyTrack.durationMs) : '3:00'}</span>
        </div>
      </div>

      {/* EQ Audio Visualizer Graphic */}
      <div className="hidden md:flex items-end gap-[3px] h-7 px-3 bg-slate-950/40 border border-slate-900/60 rounded-md shrink-0">
        {bars.map((height, idx) => (
          <div 
            key={idx}
      ...
      {/* Volume Controller (Far Right, hidden on mobile screens to save massive vertical space) */}
